...
if (!(this.item.data.data.weaponType === 'martialM' || this.item.data.data.weaponType === 'simpleM')) return;
let spells = this.actor.data.data.spells;
...
switch (selectedOption) {
	case 'p':
		update = {'data.spells.pact.value': pactSlots - 1};
		damageDiceNum = pactLevel + 1;
		break;
	case 1:
		update = {'data.spells.spell1.value': spell1 - 1};
		damageDiceNum = 2;
		break;
	case 2:
		update = {'data.spells.spell2.value': spell2 - 1};
		damageDiceNum = 3;
		break;
	case 3:
		update = {'data.spells.spell3.value': spell3 - 1};
		damageDiceNum = 4;
		break;
	case 4:
		update = {'data.spells.spell4.value': spell4 - 1};
		damageDiceNum = 5;
		break;
	case 5:
		update = {'data.spells.spell5.value': spell5 - 1};
		damageDiceNum = 5;
		break;
	case 6:
		update = {'data.spells.spell6.value': spell6 - 1};
		damageDiceNum = 5;
		break;
	case 7:
		update = {'data.spells.spell7.value': spell7 - 1};
		damageDiceNum = 5;
		break;
	case 8:
		update = {'data.spells.spell8.value': spell8 - 1};
		damageDiceNum = 5;
		break;
	case 9:
		update = {'data.spells.spell9.value': spell9 - 1};
		damageDiceNum = 5;
		break;
}
debugger;
...
let type = targetActor.data.data.details.type;
if (!type) {
	type = targetActor.data.data.details.race.toLowerCase();
} else if (type) {
	type = targetActor.data.data.details.type.value;
}
...
//await feature.roll();
let strOutput = `Lays down a mighty divine strike against ${targetToken.name} for an additional ${damageDice} damage!`;
ChatMessage.create({ speaker: { alias: actor.name }, type: CONST.CHAT_MESSAGE_TYPES.EMOTE, content: strOutput });
